...
import java.lang.*;//Binhong remove duplication by function (arraycopy)
...
public class GBManager {
	...
	public Library makeLibrary(int id) {
		...
		//int libraryID=sc.nextInt();//only for (lib-ID) included file
		int[] books = new int[numBooks];

		//Jihun regular scanning book version
		for(int i=0; i<numBooks;i++) { //every book goes into books[]
		books[i]= sc.nextInt();
		}
		sortBooks(books);

		// TODO remove duplicate?? try by Binhong
/*		for(int k=0; k<books.length; k++){
			//System.out.print("sup");
			if(!uniqueBooks.contains(books[k])){
				System.out.print("lalala");
				continue;
			}else if(uniqueBooks.contains(books[k]) == true) {
				removeElement(books, books[k]);
				System.out.print("shashasha");
			}
		}//does not working, since we should not remove duplicated at 1st library-making*/

		//TODO add to uniqueBooks set for split outout, dont forget remove it for regular scan()
	...
	/*
	//wrong approaching for remove duplicate 1st scanning by Binhong
...
//		libraries.forEach(library -> System.out.println(getAveScore(library)));
		...
		//TODO only for print sortedlibrary
		public void printSortedLibrary() {
			for(int i=0; i<libraries.size(); i++){
				...
				System.out.println();
				System.out.print(libraries.get(i).numBooks + " ");
				System.out.print(libraries.get(i).signUpDays + " ");
				System.out.print(libraries.get(i).shipDays + " ");
				System.out.print(libraries.get(i).libraryID);
				System.out.println();
			}
		}


	///jihun=good for rank2700s without greedy resorting
//	public void scan() {
//		int totalDays= days;
//		int libCount = 0;
//		int noLib=0;
...
//	}
	...
	public void setUp()  {
		...
		sortLibraries();
		printSortedLibrary();
		//scan();//common it for regular sorting library printout!!! need this for regular scan later
